...
Like1 <- function(theta){
  #mobility
  R_daily <-  rep(1,nrow(M)) %*% t(theta[(1-1)*N_geo+ (1:N_geo)]) #Rt_fun(theta = theta, x = M )
  Rt <- H %*% R_daily

  # logL <- colSums(dpois(x = mD, lambda = Rt*Ot, log = TRUE) ,na.rm=TRUE)
  # logL <- colSums(dnbinom(x = mD, mu = Rt*Ot, size =  theta[1*N_geo+1], log = TRUE) ,na.rm=TRUE)
  logL <- colSums(dnbinom(x = mD, mu = Rt*Ot, size = sqrt(Rt*Ot)* theta[1*N_geo+1], log = TRUE) ,na.rm=TRUE)
