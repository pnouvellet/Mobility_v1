...
for (date_num in length(dates_input)){
    ...
    input <- readRDS(file=paste0('../inputProcessing/Rdata/',date_week_finishing,'_inputs_SI_',1,'.rds'))
...
for (date_num in length(dates_input)){
    ...
    date_week_finishing <-  as.Date(dates_input[date_num],format = '%Y-%m-%d')
    input <- readRDS(file=paste0('../inputProcessing/Rdata/',date_week_finishing,'_inputsG_SI_',1,'.rds'))
